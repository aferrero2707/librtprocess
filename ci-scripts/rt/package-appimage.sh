...
export GIT_DESCRIBE=$(cd /work/RawTherapee && git describe)
echo "LIBRT_BRANCH: ${LIBRT_BRANCH}"
...
if [ "x${LIBRT_BRANCH}" = "xreleases" ]; then
	rtver=$(cat AboutThisBuild.txt | grep "Version:" | head -n 1 | cut -d" " -f 2)
	ver="${rtver}-$(date '+%Y%m%d_%H%M')"
else
	ver="git-${LIBRT_BRANCH}-$(date '+%Y%m%d_%H%M')"
fi
export ARCH="x86_64"
export VERSION="${ver}"
export VERSION2="librt-${LIBRT_BRANCH}-$(date '+%Y%m%d')"
