...
export PKG_CONFIG_PATH="/usr/local/lib64/pkgconfig:/usr/local/lib/pkgconfig:/usr/lib/pkgconfig:${PKG_CONFIG_PATH}"
...
echo "########################################################################"
echo ""
echo "Building and installing LibRTprocess"
echo ""

(cd "/sources" && rm -rf build && mkdir build && cd build && cmake3 -DCMAKE_INSTALL_PREFIX="/usr/local" -DPROC_TARGET_NUMBER="1" .. && make install) || exit 1
...
if [ ! -e RawTherapee ]; then
	git clone -b "${RT_BRANCH}" https://github.com/Beep6581/RawTherapee.git || exit 1
fi
cd RawTherapee || exit 1
